...
export default function LandingPageMap() {
  ...
  React.useEffect(() => {
    ...
    const handleTitleClick = event => {
        navigate('/location/' + event.target.id)
    }
                    ...
                    >
                     ...
                     <div class="hidden max-w-l mx-auto popup z-20" id={location.id}>
                        <div class="bg-white shadow-md border border-gray-200 rounded-lg max-w-sm dark:bg-gray-800 dark:border-gray-700">
                            <a href="#">
                                <img class="rounded-t-lg" src={`data:image/jpeg;base64,${location.imageData}`} alt=""/>
                            </a>
                            <div class="p-5">
                                <a href="#">
                                    <h5 id={location.id} onClick={handleTitleClick} class="text-gray-900 font-bold text-2xl tracking-tight mb-2 dark:text-white">{location.title}</h5>
                                </a>
                                <p class="font-normal text-gray-700 mb-3 dark:text-gray-400">{location.message}</p>
